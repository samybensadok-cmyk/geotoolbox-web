...
Yes, DeepSeek is free to use through the web app and mobile apps, no payment required. Where it really stands out is the API, which is priced well below the American labs. The figures below are the [current V4 API rates](https://api-docs.deepseek.com/quick_start/pricing); they change, and DeepSeek is retiring its older model aliases in mid-2026, so confirm before you build.
...
One change to plan for arrives with V4's scheduled mid-July 2026 release: DeepSeek is introducing peak/off-peak API pricing. Requests during Beijing peak hours, 09:00–12:00 and 14:00–18:00, are billed at roughly double the off-peak rate shown above, while everything outside those windows stays at the base price; DeepSeek gives 24 hours' email notice before any billing change. Confirm the current rates and windows on the [official pricing page](https://api-docs.deepseek.com/quick_start/pricing) before you build.
