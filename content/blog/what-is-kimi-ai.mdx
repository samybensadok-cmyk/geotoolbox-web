...
**Kimi is two things under one name: an AI assistant you can chat with at [kimi.com](https://www.kimi.com/en), and the family of [large language models](https://geotoolbox.ai/glossary/large-language-model) that power it, now led by Kimi K3, both made by Moonshot AI.** You use the assistant the way you use ChatGPT or Claude: ask a question, paste a document, hand it a task, and it answers in natural language. It also reads images, writes and runs code, searches the live web, and runs multi-step agent workflows.
