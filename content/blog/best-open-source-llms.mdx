...
date: "2026-07-22"
updated: "2026-08-30"
recheckBy: "2026-09-14"  # Qwen3.8-Max weights shipped 2026-08-12 (text-only, bespoke license; naming note + footnote updated 08-14, ranking unchanged: outside the permissive line). K3 note unchanged
...
Picking the best open source LLM in 2026 has a specific problem: the answer changes monthly, and most of the rankings you will find were right when they were written and wrong by the time you read them. This one is dated on every claim, against the update stamp at the top of this page (the Kimi K3 ranking and license were last re-checked on August 1, 2026, and the Qwen3.8-Max status on August 8, 2026), and honest about which numbers come from vendor decks versus verified model cards. Ten models made the cut. Their licenses differ more than their benchmark scores do, and only a handful run on hardware you can actually own.

## The Best Open-Source LLMs in August 2026, at a Glance

The short answer: [**GLM-5.3**](https://geotoolbox.ai/blog/what-is-glm-5-3) is the best open-source LLM you can download and run today, **DeepSeek V4** is the best value, and **Kimi K3** shares the benchmark lead of the open field. GLM-5.3 and Kimi K3 are tied at the top of Artificial Analysis's Intelligence Index, but K3's 2.8-trillion-parameter scale keeps it out of self-hosting reach for almost everyone, so GLM-5.3, at 753B, is the strongest open model you can actually run. One caveat on GLM-5.3: unlike GLM-5.2, it ships under a custom license, not MIT (only the smaller GLM-5.3-Flash kept MIT). Which one you should use depends on the job, the license terms you can live with, and the hardware you have.

Here is the full field, verified against official model cards and launch documentation on August 30, 2026.
...
Every ranking below rests on official model cards and launch documentation, cross-checked against independent leaderboards that publish their setups, with every claim re-verified the week of August 30, 2026. That last part matters more than it should, and here is why.
...
<tr><td>Frontier scale, newest weights</td><td><strong>Kimi K3</strong> (from July 27)</td><td>Shares the open-weight lead on the AA Intelligence Index (60, tied with GLM-5.3); 2.8T parameters</td></tr>
...
<tr><td>Multi-GPU server (300GB+)</td><td>The frontier tier, quantized</td><td>GLM-5.3, Kimi K2.6, DeepSeek V4 Pro</td></tr>
...
The lineage table below is the fix. It names the current flagship per lab, verified August 30, 2026, next to the superseded versions you will still see recommended elsewhere.

<table>
<thead>
<tr><th>Lab</th><th>Current flagship (August 2026)</th><th>Superseded, still widely recommended</th></tr>
</thead>
<tbody>
...
<figure className="not-prose my-8">
  ...
  <figcaption className="mt-3 text-center text-sm text-gray-500">Six of these labs shipped a new flagship in the last six months. Dates from official model cards and launch coverage, verified August 30, 2026.</figcaption>
</figure>
...
It depends on what you weigh. On raw benchmarks, Kimi K3 and GLM-5.3 are tied at the top of the open-weight field: as of August 30, 2026 both score 60 on Artificial Analysis's Intelligence Index (behind only Opus 5 at 63 overall), after AA's updated numbers for the field. But K3's 2.8-trillion-parameter size makes it effectively impossible to self-host, so GLM-5.3, at 753B and with a memory footprint you can actually fit on a multi-GPU server, remains the strongest open model most teams can download and run today. Note that GLM-5.3 ships under a custom license rather than GLM-5.2's MIT.
...
The weights are free; the compute is not. A 7B-12B model runs on a consumer GPU you may already own. The 24GB-GPU class covers models up to roughly 32B. Frontier models like GLM-5.3 or Kimi K2.6 need hundreds of gigabytes of memory, which means a multi-GPU server or a hosted API. For most workloads, per-token API pricing (from cents per million tokens for DeepSeek V4 Flash) is cheaper than owning hardware.
