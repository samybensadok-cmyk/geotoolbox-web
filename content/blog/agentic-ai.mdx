...
So is ChatGPT an agentic AI? On its own, no. The base model is generative AI. Switch on ChatGPT Work, OpenAI's agentic mode (launched in July 2026, gradually replacing the original "agent mode" since the following month), and connect it to tools, and the same model starts to behave agentically. The label depends on what the system can do, not on the brand name. The mechanics are broken down further down this article.
...
2. **Reason and plan.** A [large language model](https://geotoolbox.ai/blog/how-does-chatgpt-work) interprets that information, works out what the goal needs, and decides on the next step.
