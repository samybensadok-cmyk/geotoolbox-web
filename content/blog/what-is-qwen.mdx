...
date: "2026-06-22"
updated: "2026-08-07"
recheckBy: "2026-08-14"  # volatile: Qwen3.8-Max went GA 2026-08-03 and Alibaba committed to its open weights in the week of 2026-08-10 with no license named; a drop changes the Max-tier row, the licence table, the FAQ, AND requires regenerating public/blog/what-is-qwen/qwen-open-family-closed-flagship.png (it renders the PROMISED badge + the Aug 10 window) plus both EN and FR alt texts
...
<tr><td><strong>Qwen3.8-Max</strong></td><td>The current flagship, 2.4T parameters with 95B active, 1M context, multimodal</td><td>Not yet; Alibaba has committed to publishing them in the week of August 10, 2026, license unannounced</td></tr>
...
<figure className="not-prose my-8">
  <BlogImage src="/blog/what-is-qwen/qwen-open-family-closed-flagship.png" alt="Checklist of Qwen model lines as of August 2026: Qwen3.6 ships under Apache 2.0 and Qwen-Coder, Qwen-VL and Qwen-Image have open versions, while the flagship Qwen3.8-Max is closed with its weights promised for the week of August 10, and Qwen3.7-Max and Qwen3.7-Plus are closed and API-only." w="1100" h="852" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
  <figcaption className="mt-3 text-center text-sm text-gray-500">Most Qwen lines are downloadable, but the Max and Plus tiers are not, and the current flagship's open weights are still a promise rather than a download.</figcaption>
</figure>
...
Here is the honest version. Most of the Qwen lineup, the dense models and many of the Mixture-of-Experts models, ships as open weights under the permissive Apache 2.0 license. You can download them, run them on your own hardware, fine-tune them, and use them commercially, subject only to the light conditions of the Apache 2.0 license. That is the part that earned Qwen its reputation. But the flagship "-Max" tier, from [Qwen3-Max through Qwen3.7-Max and the current Qwen3.8-Max, is proprietary](https://en.wikipedia.org/wiki/Qwen): it runs only through Alibaba's API, and you cannot download it. Alibaba has followed this pattern since Qwen2, opening most of the lineup while keeping its most capable model closed.

That asterisk has grown. Two generations are now closed: Qwen3.7-Max in May 2026 and Qwen3.7-Plus in early June, then Qwen3.8-Max in August, all API-only. There is still no open-weight Qwen above 3.6 of any size as of August 2026. With the 3.6 release, weights followed the API within weeks; the 3.7 line never got them at all. So the mid-tier model most builders would actually reach for is closed too, not just the flagship. The counterweight is that Alibaba has not slowed its open releases elsewhere, shipping Apache 2.0 weights for agent-environment, embedding, and robotics models through June and July 2026, with the most recent uploads to its Hugging Face account dated late July.

The 3.8 launch is where that pattern gets tested. Alibaba's release post says it will open-source the Qwen3.8-Max weights in the week of August 10, 2026, and describes it as the first time a Max-class Qwen has been opened. That would be a real break with three years of practice, which is also why it is worth checking rather than assuming: as of August 7 the weights have not appeared in [Alibaba's Hugging Face organization](https://huggingface.co/Qwen) and no license has been announced. Repositories carrying the Qwen3.8 name have shown up from third parties in the meantime, holding either nothing or older models relabelled with the new number, so only Alibaba's own account settles it.
...
<tr><td><strong>Flagship "-Max"</strong></td><td>Qwen3.8-Max</td><td>Not yet</td><td>Proprietary (API only); weights committed for the week of August 10, 2026, license unannounced</td></tr>
</tbody>
</table>
...
Mostly, with an important exception. Most of the Qwen lineup ships as open weights under the permissive Apache 2.0 license, so you can download, run, and fine-tune those models freely. But the last two generations are proprietary and API-only, from the Qwen3.7-Plus mid-tier up to the Qwen3.8-Max flagship, and the open line stops at Qwen3.6. So "is Qwen open source?" is yes for the models most people run today, and no for anything current. That may be about to change: Alibaba has committed to publishing the Qwen3.8-Max weights in the week of August 10, 2026, which would be its first open Max-tier model, though as of August 7 they have not appeared and no license has been named. As with all of these, "open weights" also is not the same as fully open source, since the training data is not released.
