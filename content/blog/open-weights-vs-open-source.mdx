...
date: "2026-07-03"
updated: "2026-08-07"
recheckBy: "2026-08-14"  # volatile: Alibaba committed to Qwen3.8-Max open weights in the week of 2026-08-10 with no license named; if they land, the Max-tier-stays-closed line in the Qwen row, the "Who Is Still Releasing Weights" section and the Qwen FAQ all need the delivered license
...
<tr><td><strong>Qwen (Alibaba)</strong></td><td>Open-weight, up to a point</td><td>Apache 2.0 (Qwen3 series, through Qwen3.6)</td><td>None on the open line, which stops at Qwen3.6; the 3.7 and 3.8 generations are API-only. Alibaba has committed to publishing weights for Qwen3.8-Max in the week of August 10, 2026, which would be its first open-weight Max-class model; no license named yet</td></tr>
...
<tr><td><strong>GLM-5.2 (Zhipu / z.ai)</strong></td><td>Open-weight</td><td>MIT</td><td>None</td></tr>
...
The more interesting question this year is not what labs call their releases but which labs are still making them. Two of the archetypal open-weight vendors have quietly moved their best work behind an API. Meta launched [Muse Spark](https://about.fb.com/news/2026/04/introducing-muse-spark-meta-superintelligence-labs/) in April 2026, its first proprietary closed-weight frontier model, available in private preview to select partners; Meta says it hopes to open-source future versions, without committing to a timeline. Alibaba did the same across two straight generations: Qwen3.7-Max in May, the Qwen3.7-Plus mid-tier in June, and [Qwen3.8-Max](https://geotoolbox.ai/blog/qwen3-8-max) in August are all API-only, while the downloadable Qwen line stops at 3.6. OpenAI's gpt-oss weights have not been refreshed since their August 2025 release. In each case the older weights stay up and nothing is formally discontinued, but the frontier moved closed.

Alibaba is the one to watch, because it has promised to reverse its own pattern. The Qwen3.8-Max launch post commits to publishing the weights in the week of August 10, 2026, and calls it the first time it will have open-sourced a Max-class Qwen model. As of August 7 nothing has appeared in Alibaba's Hugging Face organization and no license has been named, so the promise is still a promise. A lab reopening its frontier would be the first real counterexample to the direction this section describes.
...
### Is Qwen open source?
Partly. Qwen's open-weight models, the Qwen3 series through Qwen3.6, are released under Apache 2.0, so they are free for commercial use with no user cap. Older Qwen models used a custom community license with more conditions. But Alibaba's last two generations are closed and API-only, from the Qwen3.7-Plus mid-tier up to the current Qwen3.8-Max flagship, so the open line and the frontier line have diverged. Alibaba has committed to publishing Qwen3.8-Max weights in the week of August 10, 2026, which would be its first open Max-class model; as of August 7 they have not appeared and no license has been named. And even if they land, they would only make Qwen open-weight, not open source: Alibaba has promised no training code and no data disclosure, and the license is still unknown.
