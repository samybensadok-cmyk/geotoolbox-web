...
date: "2026-07-22"
updated: "2026-08-07"
recheckBy: "2026-08-14"  # volatile: Alibaba committed to the open weights in the week of 2026-08-10 with no license named; a drop rewrites the weights section, the comparison table and two FAQ answers
...
This page pins down what is actually known as of August 7, 2026: what the published benchmarks show and where the outside measurements disagree, what the API costs, whether the promised open weights have arrived, and why a model most marketers will never touch still shapes what AI tools say about their brands. Every number below is labeled as Alibaba's own or independently measured.
...
The competitive timing was not subtle. The July preview landed days after Moonshot AI announced [Kimi K3](https://geotoolbox.ai/blog/what-is-kimi-k3), the model Moonshot's own card puts at 2.8T parameters, which had just claimed the "largest ever" headline and then shipped its weights on schedule. Alibaba has now promised something it has never done: open weights for a Max-class model, committed for the week of August 10, 2026. For a company whose [Qwen family](https://geotoolbox.ai/blog/what-is-qwen) built its reputation on open models, that promise is doing a lot of work, and we get to its track record below.
...
The July preview shipped with almost nothing you could check. The August release fixed most of that, and the distinction that still matters is between what Alibaba has now published and what anyone outside Alibaba has confirmed. The table below shows where each spec stands as of August 7, 2026.
...
<tr><td><strong>Open-weight release</strong></td><td>Promised, not delivered</td><td>Committed to the week of August 10, 2026. As of August 7 nothing has appeared on Hugging Face or ModelScope and no license has been named</td></tr>
</tbody>
</table>
...
<tbody>
<tr><td><strong>Qwen3.8-Max</strong></td><td>2.4T total, 95B active (vendor-stated)</td><td>Vendor table: 86.6 Terminal-Bench 2.1, 67.7 SWE-bench Pro, 92.6 GPQA. Independently, Artificial Analysis measures 81.3 on Terminal-Bench 2.1</td><td>$2 in / $6 out</td><td>Closed; committed for the week of August 10, 2026, license unannounced</td></tr>
<tr><td><strong>Kimi K3</strong></td><td>2.8T (claimed)</td><td>StackPerf 83/100; AA Intelligence Index 57</td><td>$3 in / $15 out</td><td>Published on Hugging Face under a custom Kimi K3 License</td></tr>
...
The open-weight promise is the biggest claim in the release and the one with the weakest track record behind it. **As of August 7, 2026 the weights have not shipped.** Alibaba's August 3 launch post commits to publishing them "next week," which is the week of August 10, and names no license.

Here is the pattern it would break. Alibaba runs a genuine two-track strategy: the small and mid-size Qwen models ship under Apache 2.0 and dominate the open ecosystem, while the Max-tier flagships stay closed. Qwen3.7-Max: API-only. Qwen3.6-Max-Preview: API-only. No Max-tier flagship has ever had its weights published, which Alibaba itself acknowledges by calling this "the first time we will open-source the weights of a Qwen-Max-class model." A named window is a real upgrade on July's "soon," but a window is not a repository.

The competitive read is hard to ignore: Moonshot announced Kimi K3 with a firm weights date of July 27, days before Alibaba's preview, and then met it. A promise keeps Alibaba's open-flagship credibility alive through a news cycle it would otherwise have lost, and weight-release promises across the industry have a habit of slipping once headlines move on.
...
The realistic play for anyone who [runs models locally](https://geotoolbox.ai/blog/run-llm-locally) is to wait for the family, not the flagship. Qwen's actual open-weight strength has always been its smaller checkpoints, the 7B-to-480B range that much of the local agent ecosystem is built on. That is why the more useful half of the promised release is the smaller one. On launch day the Qwen team said that alongside the Max weights, "Qwen3.8-27B is also going open-weights," and a 27-billion-parameter model is something a single well-specified machine can actually run. That commitment was made on Alibaba's X account rather than in the launch post, which names only the Max weights. The 2.4T model itself, open or not, is a datacenter artifact.
...
Neither, today. The API is paid at $2 per million input tokens and $6 per million output, with a one-million-token free allowance to start, and the weights are still closed. Alibaba has committed to publishing them in the week of August 10, 2026, which would be the first open Max-tier Qwen ever, but as of August 7 nothing has appeared and no license has been named.
...
Alibaba has committed to the week of August 10, 2026. As of August 7 they have not been published and no license has been announced. The credible bar is a repository under Alibaba's own Hugging Face organization containing weights and a license file. Third-party repos carrying the Qwen3.8 name do not count, whether they are empty or hold relabelled weights from older Qwen models. For contrast, Moonshot named July 27 for Kimi K3's weights at announcement and delivered on it.
