...
<figure className="not-prose my-8">
  <BlogImage src="/blog/query-fan-out/how-query-fan-out-works.png" alt="Four-step flow of query fan-out: decomposition, parallel retrieval, merging, and cited synthesis." w="1100" h="671" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
  <figcaption className="mt-3 text-center text-sm text-gray-500">Fan-out in four moves: one question becomes many searches, then one cited answer.</figcaption>
</figure>
...
The most recent data complicates that, though. Ahrefs' [March 2026 analysis](https://ahrefs.com/blog/ai-overview-citations-top-10) of 863,000 keyword SERPs and 4 million AI Overview URLs found only 38% of cited pages also ranked in Google's top 10 for the same query, against roughly 76% in its July 2025 study, with the rest split almost evenly between positions 11-100 and everything beyond position 100. Read that fall carefully before repeating it: Ahrefs also improved its citation parsing between the two studies and can now see more of the citations that appear, so the two datasets are not strictly comparable, and nobody has separated how much of the drop is real movement from how much is better detection. Ahrefs itself reads the shift as fan-out at work, with AI Overviews leaning less on the direct SERP and more on the sources that surface in fan-out sub-query results, and suggests Gemini 3, behind AI Overviews since January 2026, may be expanding queries more aggressively or more broadly, pulling from related SERPs where fewer pages rank for the original query.

The two numbers are not in conflict, and they differ on two axes worth keeping straight. They measure different engines, first of all: the 43.2% is ChatGPT, the 38% is Google's AI Overviews, and the two build their source pools differently. They also measure opposite directions. 43.2% is the chance that a page ranking #1 gets cited; 38% is the share of citations that came from a top-10 page. Ranking first still lifts your odds in any individual contest. What fan-out changes is which pages can win one at all: in that March 2026 snapshot, roughly six citations in ten came from pages that ranked far down, or not at all, for the query anyone typed.
...
- Google: AI Mode in Google Search, I/O 2025 update - `blog.google/products-and-platforms/products/search/google-search-ai-mode-update`
- Search Engine Journal: Google I/O 2026, Gemini 3.5 Flash becomes the default model in AI Mode - `searchenginejournal.com/google-adds-ai-agents-to-search-redesigns-search-box-at-i-o/575311`
- Google Patents: Generating query variants using a trained generative model (US11663201B2) - `patents.google.com/patent/US11663201B2/en`
- Google Patents: Search with stateful chat (US20240289407A1) - `patents.google.com/patent/US20240289407A1/en`
- Dan Petrovic (dejan.ai): Google's Query Fan-Out System, A Technical Overview - `dejan.ai/blog/googles-query-fan-out-system-a-technical-overview`
- Semrush: We Tested Query Fan-Out Optimization - `semrush.com/blog/query-fan-out-experiment`
- Cyrus Shepard (Zyppy Signal): Fan-out Framework, 5 Steps to Improve SEO and AI Visibility - `signal.zyppy.com/p/fan-out-framework`
- Google Search Central: Generative AI performance reports in Search Console - `developers.google.com/search/blog/2026/06/gen-ai-performance-reports`
- Bing Webmaster Tools: Introducing AI Performance (grounding queries) - `blogs.bing.com/webmaster/February-2026/Introducing-AI-Performance-in-Bing-Webmaster-Tools-Public-Preview`
- Seer Interactive: Gemini query fan-out research - `seerinteractive.com/insights/gemini-3-query-fan-outs-research`
- Nectiv: What we learned analyzing 60K+ Google fan-out queries - `nectivdigital.com/blog/new-research-we-analyzed-60k-google-fan-out-queries`
- Ahrefs: What is query fan-out? - `ahrefs.com/blog/query-fan-out`
- Ahrefs: Only 38% of AI Overview citations come from top 10 pages (863K keyword SERPs, 4M AI Overview URLs; parsing improved since the July 2025 study) - 2026-03-02 - `ahrefs.com/blog/ai-overview-citations-top-10`
- Google: Spam policies for Google web search - `developers.google.com/search/docs/essentials/spam-policies`
