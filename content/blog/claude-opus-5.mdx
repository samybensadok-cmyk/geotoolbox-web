...
date: "2026-07-25"
updated: "2026-09-25"
recheckBy: "2026-10-25"  # updated 2026-09-25: added Opus 5.5 supersession note (Sep 22, $4/$20) + fixed "current default" claims (Claude Code 2.1.280: default = Opus 5.5 on Pro, Max, Team, Enterprise); 2026-09-26: 1M on every plan incl. Pro per model-config. Re-verify Anthropic's "highest available capability" guidance
...
**Update, September 22, 2026:** [Claude Opus 5.5](https://www.anthropic.com/news/claude-opus-5-5) has superseded Opus 5 as Anthropic's current Opus model, at $4 per million input tokens and $20 per million output tokens, below Opus 5's $5 and $25. Opus 5 remains available as the prior model at its original price. Everything below describes Opus 5 as it launched and applies to that model specifically; see our [Claude pricing guide](https://geotoolbox.ai/blog/claude-pricing) for the current lineup.
...
It is a [reasoning model](https://geotoolbox.ai/blog/how-ai-models-think), and at launch it was the default model on Claude Max and the strongest model available on Claude Pro; Opus 5.5 has since taken over both roles, and since Claude Code 2.1.280 the default model is Opus 5.5 on Pro, Max, Team and Enterprise (Pro and Team Standard previously defaulted to Sonnet 5). It is not on the free tier.
...
**You may not get the 1M context you read about.** Day-one users reported seeing roughly 200K through Claude Code, and at launch the explanation was in the Claude Code docs rather than the launch post: on Max, Team and Enterprise plans, Opus was "automatically upgraded to 1M context with no additional configuration," while on Pro extended context "requires usage credits." That plan split no longer applies. Anthropic's current [model configuration docs](https://code.claude.com/docs/en/model-config) (checked September 26, 2026) say Opus 4.7 and later, Opus 5 included, run with the 1M window on every plan, including Pro, with no `[1m]` variant and no usage credits. To run Opus 5 itself rather than the `opus` alias, which now resolves to Opus 5.5, use `/model claude-opus-5`. An admin can still switch 1M off entirely with `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`, which holds native-1M models to 200K. The window itself carries no price premium beyond 200K.

So the 1M figure is real, and on current Claude Code builds it no longer depends on your plan. If long-context behavior matters, check which model you are actually on before committing, and see our guide to [context window limits](https://geotoolbox.ai/blog/claude-code-context-window) for why the advertised [context window](https://geotoolbox.ai/blog/how-does-ai-search-work) and the usable one so often diverge.
...
No. Claude Opus 5 requires a paid plan. At launch it was the default model on Claude Max and the strongest model available on Claude Pro; Opus 5.5 now holds both positions, and since Claude Code 2.1.280 it is the default model on Pro, Max, Team and Enterprise. Opus 5 itself is still available on Team, Enterprise, Claude Code and the API as the prior model. Free users get Claude Sonnet 5 instead.
...
Yes, and `high` is the default there as well. On the Anthropic API, Opus 5 runs with the 1M window on every paid plan, including Pro, with no `[1m]` variant and no usage credits (at launch, Pro needed usage credits for it). Select it with `/model claude-opus-5`, since the `opus` alias now points to Opus 5.5; the window carries no price premium beyond 200K.
...
Opus 5 is not sold separately. It came with Claude Pro at $20 a month, where it was the strongest model available, and with Claude Max at $100 or $200 a month, where it was the default; Opus 5.5 has since taken over as the default model on Pro, Max, Team and Enterprise. Team and Enterprise plans include Opus too. Paying by the token through the API instead costs $5 per million input and $25 per million output for Opus 5 specifically, on a completely separate bill from the subscription.

## More Questions

### Is Claude Opus 5 better than Claude Fable 5?

On most of Anthropic's own published benchmarks, yes, at half the token price. But Anthropic still directs workloads needing the highest available capability to the Fable tier, now [Fable 5.1](https://geotoolbox.ai/blog/claude-fable-5-1) since September 1, 2026, and states that Opus 5 is not more capable overall. The practical approach is to default to Opus 5 and escalate to Fable 5 only when Opus 5 measurably fails on your specific workload.
...
- Claude Opus 5.5 - Anthropic, September 22, 2026 - `anthropic.com/news/claude-opus-5-5`
- Claude Code changelog (2.1.280) - Anthropic - `code.claude.com/docs/en/changelog`
- Model configuration (Claude Code Docs) - Anthropic, checked September 26, 2026 - `code.claude.com/docs/en/model-config`
