...
date: "2026-06-12"
updated: "2026-09-19"  # ads freshness: Germany/Europe rollout (31 mkts, self-serve 31 Aug), OAI-AdsBot, Sponsored Agents; 2nd figure added; synced during DE localization
...
<figure className="not-prose my-8">
  <BlogImage src="/blog/how-does-chatgpt-work/chatgpt-next-token-loop.png" alt="ChatGPT's four-step loop: tokenization, embeddings, attention stack, next-token pick." w="1300" h="537" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
  <figcaption className="mt-3 text-center text-sm text-gray-500">Every ChatGPT answer is this loop: tokens in, one predicted token out, repeated a few hundred times.</figcaption>
</figure>
...
Since ads launched, OpenAI runs a fourth crawler: **OAI-AdsBot** validates the safety of pages submitted as ads. It has no bearing on organic retrieval, OAI-SearchBot and ChatGPT-User still gate that, but anyone running ChatGPT Ads will find it in their own server logs.

<figure className="not-prose my-8">
  <BlogImage src="/blog/how-does-chatgpt-work/parametric-vs-retrieval.png" alt="Parametric knowledge versus live retrieval: what each is, when it runs, how current it stays, and which crawler gates it." w="1300" h="591" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
  <figcaption className="mt-3 text-center text-sm text-gray-500">Two tracks, two crawlers: parametric knowledge is always on, live retrieval only runs on demand.</figcaption>
</figure>
...
The same goes for the hope that you can pay your way in. OpenAI [began showing labeled ads in ChatGPT in the US on 9 February 2026](https://help.openai.com/en/articles/20001047-ads-in-chatgpt), opened a self-serve Ads Manager by mid-2026 (OpenAI describes the mechanism as [a relevance-weighted, second-price auction](https://help.openai.com/en/articles/20001207-ads-in-chatgpt-the-basics)), and [extended ads to 31 European markets in late August 2026](https://openai.com/index/chatgpt-ads-expands-across-europe/). But those ads sit in a labeled block apart from the answer, never inside it: OpenAI's own documentation says ads "can appear below the end of a response" and are "visually separated from ChatGPT's response," and that advertisers have "no ability to shape, rank, or alter ChatGPT's responses." As of September 2026 there is still no auction that drops your brand inside the generated text itself. The closest thing is [Sponsored Agents](https://openai.com/index/reimagining-advertising-with-ai/), in US testing since 16 September 2026, which let someone start a separate, clearly labeled conversation with a brand's agent after clicking an ad, still outside the answer, and still after a click. The answer comes out of the weights and, sometimes, a live fetch, and there is no checkout for either.
...
- The Verge: Sam Altman on the 100 trillion parameter rumor (January 2023) - `theverge.com/2023/1/18/23560328/openai-gpt4-rumor-release-sam-altman-interview`
