...
ChatGPT pricing moved more than once in 2026. Go went global in January, a second Pro tier appeared in April, OpenAI cut Business pricing that same month, the GPT-5.6 models (Sol, Terra, Luna) previewed in June and became generally available on July 9, and OpenAI cut GPT-5.6 API prices on July 30. Most pricing guides you will find still quote a tier list that has changed. Below is every current price, reconciled and dated August 2026, plus the question the numbers exist to answer: which plan, if any, you should actually pay for.
...
<tbody>
<tr><td><strong>Free</strong></td><td>$0/mo</td><td>Latest models, tight limits, ads in some markets</td><td>Casual, occasional use</td></tr>
...
Free is a real product, not a locked demo. You get limited access to GPT-5.5 Instant, which is still the default model for everyday answers, plus image generation, voice, and file uploads, with caps on how much you can do in a given window. GPT-5.6 Terra, the balanced tier of the current generation, does reach Free and Go, but only inside Codex, not in the main chat window, where neither tier gets the GPT-5.6 family at all. Almost no guide makes that distinction. When you hit a limit, ChatGPT quietly drops you to a lighter model rather than cutting you off, so the ceiling shows up as a quality dip on hard tasks rather than an error message. The full Deep Research tool is not part of Free, though the tier does include five lightweight research runs a month.
...
A note on per-window message caps. OpenAI used not to publish them, and for a long time any exact figure was guesswork. That changed: its help centre now states that Plus and Go users can send up to 160 messages with GPT-5.5 Instant every three hours, after which chats fall back to GPT-5.5 Instant mini, and that Go users get 10 Thinking messages every five hours. Reasoning limits are metered separately and are still unpublished.
