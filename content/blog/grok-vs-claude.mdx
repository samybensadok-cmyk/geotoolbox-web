...
On the API the gap is still large. Per [xAI's pricing](https://docs.x.ai/docs/models), Grok 4.5 runs $2 per million input tokens and $6 per million output tokens, while per [Anthropic's pricing](https://claude.com/pricing) Claude Opus 4.8 is $5 and $25, well over double on input and roughly four times on output. So Grok 4.5 gives you Opus-class capability by xAI's billing at more than 60% less: pricier than the prior Grok 4.3, but still far under Opus. If you are generating at volume, that difference compounds fast and Grok is the cheaper engine on sticker rates, though caching discounts and how many reasoning tokens each model burns can narrow the real-world gap.
