...
<tr><td><strong>Models</strong></td><td>Multi-model: Claude (Sonnet, Opus), GPT, Gemini, and more</td><td>Claude only (Opus 5.5, Sonnet 5, Haiku 4.5)</td></tr>
<tr><td><strong>Context window</strong></td><td>Up to 1M for supported models in VS Code and the CLI</td><td>1M in Claude Code on every paid plan and the API for current models (Opus 5.5, Sonnet 5, Fable 5.1)</td></tr>
...
The context window is where competing articles openly contradict each other, with some claiming 1M tokens and others 200k. The 200k figure is out of date for current Claude models, so here is the verified version.

Claude Code runs Opus 4.7 and later, Sonnet 5, and the Fable models with the full **1M token** window on every paid plan, Pro included, with no usage credits, per Anthropic's [model configuration docs](https://code.claude.com/docs/en/model-config) (checked September 26, 2026), and the API serves 1M at standard pricing. In the Claude chat app, Opus 5.5, Opus 5, Fable 5.1, and Sonnet 5 also get 1M on paid plans, while older models such as Opus 4.8 and Sonnet 4.6 are held to 500k and anything outside the listed models to 200k. Copilot, for its part, can use a 1M token window too, but only for supported Claude and GPT models inside VS Code and the Copilot CLI, and turning it on consumes more of your usage allowance. It is not a flat per-tier cap.
...
<tr><td><strong>Claude API</strong></td><td>pay-as-you-go</td><td>Sonnet 5 at $2/$10 per million tokens in/out; Opus 5.5 at $4/$20 (Opus 5, the prior model, remains at $5/$25)</td></tr>
</tbody>
</table>
...
<figure>
  <BlogImage src="/blog/claude-vs-copilot/claude-vs-copilot-scorecard.png" alt="Claude Code vs GitHub Copilot scorecard dated August 2026, compared across autonomy, context, IDE reach, models, and price." w="1200" h="868" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
  <figcaption className="mt-3 text-center text-sm text-gray-500">Which tool leans ahead on each dimension, as of August 2026. Since then, Pro has joined Max, Team and Enterprise: Claude Code runs 1M on every paid plan. Neither wins outright, and many developers run both.</figcaption>
</figure>
...
<tr><td>Work on a big codebase and need the largest usable context window</td><td>Claude Code (any paid plan or the API)</td></tr>
...
- How large is the context window on paid Claude plans? - Anthropic Help Center, updated September 22, 2026 - `support.claude.com/en/articles/8606394-how-large-is-the-context-window-on-paid-claude-plans`
...
- Claude Opus 5.5 is now available in GitHub Copilot - GitHub Changelog, Sept 22 2026 - `github.blog/changelog/2026-09-22-claude-opus-5-5-is-now-available-in-github-copilot`
