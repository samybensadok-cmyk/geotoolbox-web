...
A standard [transformer model](https://geotoolbox.ai/blog/how-does-chatgpt-work) generates a response one token at a time, feeding each new token back in as context for the next. That is why a long ChatGPT answer visibly streams in and why response time scales with how much the model has to say.
...
The company frames the split using Daniel Kahneman's distinction between fast, intuitive thinking and slow, deliberate thinking. A chain-of-thought [reasoning model](https://geotoolbox.ai/blog/how-ai-models-think) working through a multi-step problem is doing the slow kind.
