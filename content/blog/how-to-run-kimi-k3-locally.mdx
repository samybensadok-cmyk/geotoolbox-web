...
date: "2026-07-28"
updated: "2026-09-20"  # Sep refresh: llama.cpp mainline merged 08-15 (PR #26185), spec-decoding rollback PR #28466 09-08, Unsloth GGUF quant sizes (594GB 1-bit / 861GB 2-bit / 1.56TB Q8)
recheckBy: "2026-10-20"  # llama.cpp/GGUF support, provider throughput, and pricing all move fast on a launch-week model
...
Not at full precision on a single Mac. Even a 512GB M3 Ultra falls short of the 1.56TB the official weights need. Unsloth's 594GB 1-bit GGUF quant (about 79% accuracy retained) is the one build that fits inside a single 512GB machine with room for the KV cache, using disk or RAM offload, but that is a heavily degraded model, not the one this guide benchmarks. A cluster of four or five 512GB Mac Studios can hold the full-precision model, but the Thunderbolt link between them is about a hundred times slower than each machine's internal memory, so the interconnect sets the speed. Nobody has published a K3 run on a Mac cluster; the closest analogy, a prior-generation rig on other large models, sat around 15 tokens per second. It loads. Whether it is usable is another question.

### Do Ollama and llama.cpp support Kimi K3 yet?

llama.cpp does, Ollama still does not run it locally. Mainline llama.cpp gained K3 support via an [August 15, 2026 merge](https://github.com/ggml-org/llama.cpp/pull/26185), less than three weeks after this guide first published, so the GGUF repos no longer require compiling from a feature branch. A September 8, 2026 follow-up ([PR #28466](https://github.com/ggml-org/llama.cpp/pull/28466)) enabled K3's recurrent-state rollback during speculative decoding, a serving-performance feature vLLM and SGLang already had at their day-0 launch. Unsloth's dynamic GGUF quants run from a 594GB 1-bit build (about 79% accuracy retained) up to a lossless 1.56TB Q8 rebuild. Ollama's cloud tag still routes to Ollama's own servers rather than running locally, and there is no dedicated Ollama build yet, so llama.cpp (or a GUI built on it, like LM Studio) is the only local-Ollama-style path today.

### How big is the Kimi K3 download?

The official repository is 1.56TB across 96 weight shards, in native MXFP4 4-bit precision. The widely repeated 594GB figure is not a separate or mistaken number: it's the size of Unsloth's 1-bit dynamic GGUF quant (UD-IQ1_S), which retains roughly 79% accuracy on Unsloth's own benchmark. Because K3 was quantization-aware-trained at 4-bit, there is no free halving from the usual 16-bit-to-4-bit step; Unsloth's 2-bit build (UD-Q2_K_XL) still runs about 861GB, and its lossless Q8 rebuild matches the full 1.56TB. Sub-4-bit community quants trade quality for the reduction rather than compressing a bloated original.
...
- Kimi K3 GGUF quantizations and benchmarks - Unsloth Documentation - `unsloth.ai/docs/models/kimi-k3`
- model: add Kimi-K3 text model (PR #26185, merged August 15, 2026) - llama.cpp / GitHub - `github.com/ggml-org/llama.cpp/pull/26185`
- model: support Kimi-K3 recurrent-state rollback (PR #28466, merged September 8, 2026) - llama.cpp / GitHub - `github.com/ggml-org/llama.cpp/pull/28466`
