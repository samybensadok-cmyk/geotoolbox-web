...
It is worth being clear about the gaps, because the breathless coverage tends to paper over them. Even at general availability, OpenAI has not published a firm context window for GPT-5.6. Early write-ups cite figures from 1 million to 1.5 million tokens, but those numbers disagree with each other and none trace back to an official spec, so the honest answer today is that it is unconfirmed. The same goes for the knowledge cutoff, which OpenAI has not stated.

And while the models are now broadly usable, most of the benchmark and capability claims still trace back to OpenAI's own launch materials. Take the launch-day numbers as OpenAI's account, not as settled fact, and expect the real picture to sharpen now that outside researchers can run the models themselves.
...
OpenAI has not officially published a context window for GPT-5.6, even at general availability. Early reports cite figures between 1 million and 1.5 million tokens, but they conflict and none trace to an official spec, so treat the number as unconfirmed for now.
