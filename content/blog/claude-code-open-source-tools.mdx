...
Search for "open-source tools for Claude Code" and you get two different answers tangled together. One is tools that **replace** Claude Code: open-source CLI agents like OpenCode, Aider, and Cline that run the same job on a different (often model-agnostic) engine, up to and including a model you [run locally yourself](https://geotoolbox.ai/blog/run-llm-locally). The other is tools that **improve** the Claude Code you already use.
