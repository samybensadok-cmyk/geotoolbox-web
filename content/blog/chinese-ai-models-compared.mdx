...
date: "2026-06-27"
updated: "2026-08-18"
recheckBy: "2026-09-14"  # Qwen weights-status resolved 08-14 (shipped 08-12, text-only, bespoke license). DeepSeek price row updated to off-peak 08-18 (peak/off-peak split landed 08-16). STILL PENDING for the freshness track: Kimi row K2.6 -> K3, GLM-5.3 (released 08-14, not yet reflected here), and open-vs-closed.png regen (June-2026 capture, now date-scoped in alt+figcaption)
...
[DeepSeek's current flagship](https://geotoolbox.ai/blog/what-is-deepseek) is [V4](https://geotoolbox.ai/blog/deepseek-v4), which [TechCrunch reported was previewed in late April 2026](https://techcrunch.com/2026/04/24/deepseek-previews-new-ai-model-that-closes-the-gap-with-frontier-models/), built on the same mixture-of-experts efficiency the lab is known for; it reached general availability on July 20, 2026. It is genuinely good at reasoning and agentic coding, and on price it is close to unbeatable: the Flash tier runs around fourteen cents per million input tokens, where Western frontier models charge dollars. As of August 2026 those rates are flat all day, though DeepSeek's pricing page warns of a coming "significant" overall price increase, "subject to official notice." If your question is "which Chinese model gets me the most capability per dollar," DeepSeek is usually the answer, and it is where most DeepSeek-versus-Qwen comparisons land on cost alone.
...
The practical takeaway is that when a developer anywhere fine-tunes "an open model," it is very often a Qwen underneath. The smaller and mid-size Qwen models ship with open weights under permissive licenses, which is what makes them the default starting point. The catch: Alibaba keeps its very largest models as a closed API rather than an open download, a small but telling retreat from full openness. Qwen's strengths are breadth: strong multilingual coverage, agentic tool use, and a one-million-token context window. The current flagship, [Qwen3.8-Max](https://geotoolbox.ai/blog/qwen3-8-max), was released on August 3, 2026 at 2.4 trillion parameters with 95 billion active, priced at $2 per million input tokens and $6 per million output. Alibaba published its weights on August 12, 2026, the first open Max-tier Qwen, though as a text-only variant under a bespoke "Qwen3.8-Max" license rather than Apache 2.0.
...
<figure className="not-prose my-8">
  <BlogImage src="/blog/chinese-ai-models-compared/open-vs-closed.png" alt="Most Chinese frontier AI models ship open weights you can download and self-host; in this June 2026 capture the top Qwen, Baidu's Ernie 5.1, and ByteDance's Doubao stay API-only (since August 12, 2026 the newest Qwen Max ships text-only open weights under a bespoke license)." w="2200" h="1470" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
  <figcaption className="mt-3 text-center text-sm text-gray-500">A June 2026 capture: the open downloads spanned four flagships plus open siblings from Qwen and Ernie, while the top Qwen, Baidu's Ernie 5.1, and ByteDance's Doubao stayed closed. Since August 12, 2026, the newest Qwen Max ships text-only weights under a bespoke license.</figcaption>
</figure>
...
<tr><td><strong>Kimi K2</strong></td><td>Modified MIT</td><td>Yes</td><td>Yes</td><td>Training data and code</td></tr>
...
<tr><td><strong>Qwen (smaller models)</strong></td><td>Apache 2.0</td><td>Yes</td><td>Yes</td><td>Flagship Max weights, training data</td></tr>
