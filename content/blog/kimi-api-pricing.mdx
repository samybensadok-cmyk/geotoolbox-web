---
title: "Kimi API Pricing $MONTH_YEAR: Cost, Keys, and Limits"
description: "What the Kimi API costs in $YEAR: per-model pricing, the $1 minimum, rate-limit tiers, and the endpoint split that quietly returns a 401 on a valid key."
date: "2026-07-18"
updated: "2026-09-05"
recheckBy: "2026-10-06"  # 2026-09-05: the K2.5 + moonshot-v1 retirement LANDED on 2026-08-31 (verified live on platform.kimi.ai/docs/models — calls now return 404) and is past-tensed throughout (table status cells, the legacy-row paragraph, the reseller table and the retirement chronology all re-checked line by line on 2026-09-05); summer launch top-up rebate lapsed. Next check: per-model prices vs platform.kimi.ai/docs/pricing/*, and whether the $5 cumulative-spend voucher is still standing
...
The column most pricing roundups omit is the middle one. Moonshot charges a tenth of list for cached input on K3 and roughly a fifth to a sixth on the other K2-line models. The legacy `moonshot-v1` family got no cache discount at all before its retirement, so what you pay depends on your prompt repetition and on which model you picked.
...
<tr><td><strong>kimi-k2.5</strong></td><td>$0.60</td><td>$0.10</td><td>$3.00</td><td>262,144</td><td>Retired Aug 31, 2026 (404)</td></tr>
<tr><td><strong>moonshot-v1-128k</strong></td><td>$2.00</td><td>None</td><td>$5.00</td><td>131,072</td><td>Retired Aug 31, 2026 (404)</td></tr>
<tr><td><strong>moonshot-v1-32k</strong></td><td>$1.00</td><td>None</td><td>$3.00</td><td>32,768</td><td>Retired Aug 31, 2026 (404)</td></tr>
<tr><td><strong>moonshot-v1-8k</strong></td><td>$0.20</td><td>None</td><td>$2.00</td><td>8,192</td><td>Retired Aug 31, 2026 (404)</td></tr>
</tbody>
</table>

Prices are per million tokens and exclude tax, which Moonshot calculates at checkout based on your jurisdiction. The last four rows are kept for reference only: `kimi-k2.5` and the `moonshot-v1` family were retired on August 31, 2026 and calls to them now return a 404. You cannot buy those rates today.
...
The second is the legacy row, and it is now a cautionary tale rather than a live trap. Until August 31, 2026 the `moonshot-v1` family was still on sale at rates that made no sense: `moonshot-v1-128k` cost more per input token than `kimi-k2.6` while giving you half the context and no cache discount at all. Anyone running a 2025 integration that nobody had revisited was paying more for less, right up until the model string stopped resolving. Both families were retired on the same date, so an unmaintained integration did not quietly overpay, it broke.
...
This is the most common misconception about Kimi, and the search data shows it: most questions Google surfaces about the Kimi API are some version of "is it free?" The confusion is understandable, because the consumer Kimi app at kimi.com does have a free tier, and its paid plans are metered in credits rather than tokens. We cover that side separately in our [Kimi pricing](https://geotoolbox.ai/blog/kimi-pricing) guide. The API is a separate product with separate billing.
...
**Note:** *A paid subscription to the consumer Kimi app does not include metered API credits. Moonshot bills the API Open Platform separately from the Kimi app and Kimi Code.*
...
<tr><td><strong>OpenRouter</strong></td><td>Multi-provider fallback, card payment</td><td>Per token, pass-through plus margin</td><td>Exact parity with direct on K3; its old K2.5 discount died with the model</td></tr>
...
Before the sunset, [OpenRouter listed kimi-k2.5](https://openrouter.ai/moonshotai/kimi-k2.5) at $0.375 input and $2.025 output per million tokens against Moonshot's own $0.60 and $3.00 for the same model: 37.5% cheaper on input and 32.5% cheaper on output, through a reseller. On K3 the same comparison lands at exact parity, $3.00 and $15.00 either way, so that discount was a K2.5 artifact rather than a standing rule.

That gap is now history rather than an opportunity. K2.5 was retired on August 31, 2026, and Moonshot's [model list](https://platform.kimi.ai/docs/models) confirms that calls to `kimi-k2.5` and the `moonshot-v1` series return a 404. Any reseller listing that survived the sunset is a stale catalogue entry, not a live route. The nearest replacement, `kimi-k2.7-code`, moves cache-miss input from $0.60 to $0.95. The general lesson stands even though this particular arbitrage is gone: check reseller pricing against the vendor's own rate before assuming first-party wins, and check the model is still alive before either.
...
Fourteen model IDs have been retired since November 2025, and there is no stable alias to hide behind. For anything with a maintenance window longer than a quarter, that is a bigger operational risk than the price.

The [model list](https://platform.kimi.ai/docs/models) records the pattern. `kimi-thinking-preview` was discontinued in November 2025. `kimi-latest` went in January 2026. The entire kimi-k2 series was discontinued on May 25, 2026. `kimi-k2.5` and the whole `moonshot-v1` family, including three `-vision-preview` variants, came off on August 31, 2026.
...
Not for the metered API. Moonshot bills the API Open Platform separately from the Kimi app and Kimi Code. Kimi Code subscribers do receive their own keys, but those run against a request quota of roughly 300 to 1,200 calls per rolling five-hour window depending on plan, not a token balance. Community threads sometimes claim subscriptions grant matching API credit; Moonshot's documentation does not support that.

### Which Kimi model should I use?

For coding and agent work, kimi-k2.7-code at $0.95 input and $4.00 output is the value pick. Choose kimi-k3 when you specifically need the million-token [context window](https://geotoolbox.ai/blog/how-does-ai-search-work) or its stronger reasoning, and price it accordingly, because it costs roughly three times as much per unit of work and is not on the batch pricing table. The legacy `moonshot-v1` family is no longer an option at all: it was retired on August 31, 2026 alongside `kimi-k2.5`, and calls to it now return a 404. Some variants still carry lower list prices, but they come with far smaller contexts and almost no migration runway.
...
- Kimi K3 launch top-up rebate (dedicated page since taken down) - Moonshot AI developer docs, as captured July 18, 2026 - `platform.kimi.ai/docs/pricing/promotion`
