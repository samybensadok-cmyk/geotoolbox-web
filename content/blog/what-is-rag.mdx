...
**Retrieval-augmented generation (RAG)** is a technique that lets a large language model look information up at answer time instead of relying only on what it memorized during training. The model retrieves relevant documents, reads them, and writes an answer grounded in what it just found.
...
The term comes from a 2020 paper, [Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](https://arxiv.org/abs/2005.11401), led by Patrick Lewis with a team of machine learning researchers from Facebook AI Research (now Meta AI), University College London, and NYU. It has since been [cited more than 20,000 times](https://scholar.google.com/scholar?q=Retrieval-Augmented+Generation+for+Knowledge-Intensive+NLP+Tasks) and is the reference point everyone else builds on.
...
When ChatGPT browses the web to answer a question, it retrieves pages from a search index (built on third-party search providers), reads them, and writes a grounded reply with links. [Perplexity](https://geotoolbox.ai/blog/perplexity-seo) retrieves and cites on essentially every query. Google's AI Overviews pull candidate passages from pages that already rank, then synthesize them. Different engines, same three steps: retrieve, augment, generate. This is just [how AI search works](https://geotoolbox.ai/blog/how-does-ai-search-work) under the hood.
