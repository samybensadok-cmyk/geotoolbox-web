...
date: "2026-07-17"
recheckBy: "2026-10-09"  # 09-25: independent standings moved to AA Intelligence Index v4.3.2 (checked 09-25/09-26: K3 44, #3 open-weight behind MiMo-V2.6-Pro 46 and GLM-5.3 45); recheck for the next AA re-version and for independent K3 coding/reasoning benchmarks. Earlier: weights public (July 27); reasoning_effort low/high/max landed (confirmed 2026-08-08); Qwen3.8-Max open-weights (08-12) referenced at the K3-launch-timing line.
updated: "2026-09-25"
recheckByNote: "09-25: rescored again to v4.3.2 (K3 44, #3 open-weight, behind new Xiaomi MiMo-V2.6-Pro 46 and GLM-5.3 45); fixed the 3 spots that still asserted K3 leads/tops the open field (benchmark prose, benchmark table cell, FAQ). 09-07: fixed AA Intelligence Index (v4.1.1-stale '60, tied with GLM-5.3' -> verified v4.2 '50, GLM-5.3 49, not tied') at the 2 spots that hardcoded the number. NOT done this pass, scope boundary of a lighter-touch sweep: the article's many 'Fable 5' / 'GPT-5.6 Sol' comparisons throughout (intro quote, benchmark table, cost-per-task, recommendation table, FAQ) are anchored to specific vendor-reported numbers tied to those exact model versions and were not individually re-verified against Claude Fable 5.1 / GPT-6 Astra (both released since) - that needs its own forensic pass, flagging rather than silently leaving or overclaiming a full fix."
...
<figure className="not-prose my-8">
  <BlogImage src="/blog/what-is-kimi-k3/kimi-k3-benchmarks-vendor-vs-independent.png" alt="Table sorting Kimi K3 benchmarks by who measured them, as reported July 17, 2026: independent results (AA Intelligence Index 57, Frontend Code Arena #1, hallucination rate 51%) versus Moonshot's vendor-reported scores (GPQA Diamond 93.5, Terminal-Bench 88.3, BrowseComp 91.2)." w="1100" h="773" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
...
The results worth leaning on today are the independently checked ones. At launch, [Artificial Analysis](https://artificialanalysis.ai/models/kimi-k3) scored K3's overall Intelligence Index at 57 (the figure in the chart above, as reported July 17). The index has since been re-versioned several times, re-weightings of the benchmark suite rather than changes to the model: the August version (v4.1.1) put K3 at 60, tied with GLM-5.3; the September 4, 2026 version (v4.2) put K3 at 50, GLM-5.3 at 49, still the top open-weight model; and on the current v4.3.2 (September 2026, checked September 25) K3 scores 44, now third among open-weight models behind Xiaomi's new MiMo-V2.6-Pro (46) and GLM-5.3 (45), and still behind the closed frontier of Claude Opus 5, Anthropic's newer Claude Fable 5.1, and OpenAI's GPT-6 Astra (released September 3, 2026, superseding GPT-5.6 Sol as the closed flagship). Its long-horizon knowledge-work Elo of 1547 trailed only Fable 5 when measured at launch. Separately, the Frontend Code Arena, a human-preference leaderboard, ranks it first, ahead of Fable 5 and GPT-5.6 Sol, though critics note that leaderboard leans heavily on frontend and 3D-demo tasks, so read it as coding-flavor strength rather than general capability. Read together, those say something specific: K3 was at the frontier on narrow coding and agent tasks at launch, and has since slipped from the open-weight intelligence lead while staying competitive on coding-flavor benchmarks.
...
<tbody>
<tr><td><strong>AA Intelligence Index</strong></td><td>44 (#3 open model, v4.3.2, checked Sept 25 2026)</td><td>higher</td><td>higher</td><td><strong>Independent</strong> (Artificial Analysis)</td></tr>
...
<tr><td><strong>DeepSeek V4 Pro</strong></td><td>$0.66 off-peak / $1.32 peak</td><td>$1.98 off-peak / $3.96 peak</td><td>~$0.022</td><td>Frontier-class, a fraction of K3 per token</td></tr>
</tbody>
</table>

Whether $15 hurts depends entirely on your workload. Because K3 reasons on every request and can be verbose, a single task often drags a long thinking trace, retried tool calls, and a growing history through the output meter, so heavy output bills are routine rather than rare. On Artificial Analysis's cost-per-task measure K3 runs about $0.95, under half the $2.03 [Claude Opus 5](https://geotoolbox.ai/blog/claude-opus-5) costs at max effort and close to GPT-5.6 Sol at $1.04, which is reasonable for frontier-grade work. That Sol figure predates OpenAI's August 21, 2026 price cut, so its real cost per task now runs lower than the index shows. But if your goal was to save money by going open, note that DeepSeek V4 Pro completes a comparable task for a small fraction of that. Our [DeepSeek pricing breakdown](https://geotoolbox.ai/blog/deepseek-pricing) and [Claude pricing guide](https://geotoolbox.ai/blog/claude-pricing) give the fuller comparison.
