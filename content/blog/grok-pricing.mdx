...
The new flagship is [grok-4.5](https://geotoolbox.ai/blog/grok-4-5), released on July 8, 2026 as xAI's Opus-class coding and agentic model, at $2 input and $6 output per million tokens with a 500K-token context window. Watch the context tier, because the headline price is only half the story: xAI's docs list grok-4.5 at $2 input / $6 output for prompts **under 200K tokens**, and **$4 / $12 above that**, with cached input moving from $0.30 to $0.60. Since the model's selling point is a 500K window, any prompt that actually uses it bills at the higher rate. That is pricier than the outgoing grok-4.3 ($1.25/$2.50, also doubling above 200K), but still well under rivals like Claude Opus 4.8 ($5/$25) and GPT-5.6 Sol ($5/$30). grok-4.3 remains the value option, and at $1.25 input and $2.50 output it undercuts the comparable frontier models from OpenAI and Anthropic, whose top tiers run several times higher. That price-per-intelligence is xAI's main pitch to developers right now.
