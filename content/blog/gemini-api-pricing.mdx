...
<tr><td><strong>Priority</strong></td><td>$2.70</td><td>$16.20</td><td>Roughly 1.8x Standard, latency guarantees</td></tr>
</tbody>
</table>
...
The controls that actually cap spend are more specific. The usage-tier billing caps pause service once your cumulative spend hits the tier limit, so a Tier 1 billing account cannot run past $250 in a cycle. Inside AI Studio you can set per-project spend caps, useful when several projects share one billing account, though [Google warns](https://ai.google.dev/gemini-api/docs/billing) that batch jobs and agent sessions can overshoot a project cap slightly because billing lags real usage by about ten minutes.
...
- [Vertex AI generative AI pricing](https://cloud.google.com/gemini-enterprise-agent-platform/generative-ai/pricing) - Google Cloud (rate parity, Provisioned Throughput, caching discounts)
