...
description: "Profound pricing reconciled tier by tier: the $99 Starter, $399 Growth, and custom enterprise plans, the $MONTH_YEAR shift to public prices, and whether it is worth it."
date: "2026-06-09"
updated: "2026-08-06"
...
geotoolbox, which is ours, belongs in that cheaper tier too. Plans run from $99 a month ($79 billed annually) with a 7-day free trial, adding engines up to all eight; the entry Starter covers the same three engines (ChatGPT, Perplexity, and Google AI Overviews) that Profound's $399 Growth does. It also adds a check most trackers bury beneath the visibility numbers: whether the AI crawlers can reach your pages at all, which in our experience is a common reason a brand shows near-zero visibility, because a firewall or a JavaScript-only render quietly blocks the bots before they ever read the page. To be plain about the gaps as well as the pitch: geotoolbox has no equivalent of Profound's prompt-volume dataset or its SOC 2 enterprise posture, so weigh it against the rest on the free trial rather than on our word. The point is not that any one tool beats Profound. It is that multi-engine tracking costs $29 to $399 a month across this market, not four figures.
