...
Most "AI agent examples" lists mix two different things: chatbots that answer questions, and agents that go do something. The table below leans toward the second kind (a couple of genuinely contested entries are flagged as such), organized by what each one does rather than what cognitive architecture it uses under the hood.
...
<figure>
  <BlogImage src="/blog/ai-agent-examples/ai-agent-categories-overview.png" alt="Six AI agent categories - research, coding, customer support, sales, finance, and personal - each with named example products." w="1100" h="637" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
...
**[GitHub Copilot's coding agent](https://github.com/features/copilot/agents)** (Microsoft) takes an issue description, explores the relevant part of a repository, writes the code change, runs the test suite, and opens a pull request without a human driving each step. OpenAI's Codex does the same job through the ChatGPT interface, the command line, or an IDE plugin. Anthropic's own [Claude Code](https://geotoolbox.ai/blog/what-is-claude-code) works from a terminal: it reads your codebase, edits files across multiple locations, runs shell commands, and iterates on failing tests until the change passes.
...
Function is one way to sort agents, which is what this article does. The other common way sorts by architecture - simple reflex, goal-based, utility-based, learning, and multi-agent systems, among others - which describes how an agent decides what to do rather than what job it does. See our [glossary entry on AI agents](https://geotoolbox.ai/glossary/ai-agent) for the base definition this article builds on.
