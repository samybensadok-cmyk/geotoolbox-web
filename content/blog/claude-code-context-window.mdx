...
date: "2026-07-04"
updated: "2026-09-25"
recheckBy: "2026-11-25"  # updated 2026-09-25/26: current-Opus row now Claude Opus 5.5 (superseded Opus 5 on 2026-09-22); per code.claude.com model-config (2026-09-26) Fable 5.1/5, Sonnet 5, Opus 4.7+ run 1M on every plan incl. Pro (no credits); only Opus 4.6/Sonnet 4.6 need [1m] + plan rules. Re-verify the lineup on platform.claude.com
...
<tr><td><strong>Claude Opus 5.5</strong> (current Opus)</td><td>1M tokens</td><td>Automatic on every paid plan, including Pro, with no <code>[1m]</code> suffix and no usage credits. Opus 5, the prior model, is still selectable and keeps the same 1M behavior</td></tr>
<tr><td><strong>Fable 5.1</strong> (and Fable 5)</td><td>1M tokens</td><td>1M on every paid plan, including Pro. Fable usage itself can bill to usage credits on some plans and seat tiers</td></tr>
<tr><td><strong>Haiku 4.5</strong></td><td>200K tokens</td><td>The one current model that does not reach 1M</td></tr>
<tr><td><strong>Legacy</strong> (Opus 4.8 / 4.7 / 4.6, Sonnet 4.6)</td><td>1M tokens</td><td>Opus 4.8 and 4.7 run 1M on every paid plan, like Opus 5.5. Opus 4.6 and Sonnet 4.6 reach 1M only through their <code>[1m]</code> variant: Opus 4.6 is included on Max, Team, and Enterprise and needs usage credits on Pro, while Sonnet 4.6 requires usage credits on every subscription plan, including Max</td></tr>
</tbody>
</table>

The 1M window is available on Pro, Max, Team, and Enterprise, not the free tier, per Anthropic's [context window documentation](https://support.claude.com/en/articles/8606394-how-large-is-the-context-window-on-paid-claude-plans). The plan split, from Anthropic's [model configuration docs](https://code.claude.com/docs/en/model-config) (checked September 26, 2026): on the Anthropic API, Fable 5.1, Fable 5, [Sonnet 5](https://geotoolbox.ai/blog/claude-sonnet-5), and Opus 4.7 and later run with the 1M window on every plan, including Pro, with no configuration and no usage credits. The plan rules only apply to Opus 4.6 and Sonnet 4.6, covered in the legacy row above.

Two details that matter for cost. First, **the 1M window uses standard model pricing with no premium for tokens beyond 200K.** Where extended context is included with your subscription, it stays covered by your subscription; where it comes through usage credits, those tokens bill to credits. A bigger window costs more because you are sending more tokens, not because the tokens themselves are priced higher. Second, on current models there is nothing to select. The `[1m]` suffix only matters for the two older models that reach 1M through a separate variant:
...
If your account supports those variants, they also appear in the `/model` picker; restart the session if you do not see them. `sonnet[1m]` has no effect when `sonnet` already resolves to Sonnet 5, since that model is 1M natively.
...
<figure className="not-prose my-8">
  <BlogImage src="/blog/claude-code-context-window/context-meter-breakdown.png" alt="An annotated Claude Code /context readout for Sonnet 5 against its 1M window, showing the window split into system prompt, system tools, MCP tools, memory files, messages, free space, and the reserved autocompact buffer, each with its token count and percentage." w="2200" h="1424" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
...
The top line shows your model and usage, something like `117k/1M tokens (12%)` on a current model; the denominator reads 200k on Haiku 4.5, on Opus 4.6 or Sonnet 4.6 without their `[1m]` variant, or in the other cases listed above. Below it, each category reports its own tokens and percentage: system prompt, system tools, MCP tools, custom agents, memory files, messages, then free space and a reserved autocompact buffer at the bottom. The value is that it turns a vague "why is this session sluggish" into a specific answer. If MCP tools are showing 26k tokens, you know where to cut. If your `CLAUDE.md` is 4k before you have done anything, that is a fixed tax on every turn worth trimming.
...
On current models it is 1 million tokens. Sonnet 5 runs at 1M unconditionally, Opus 5.5 and Fable 5.1 reach 1M on paid plans, and Haiku 4.5 is the one current model still capped at the 200,000-token standard window. The usable space is always smaller than the headline number, because the system prompt, tools, memory, and your `CLAUDE.md` occupy part of the window before you type anything.

### Does Claude Code have a 1 million token context window?
Yes. On the Anthropic API, Sonnet 5, Opus 5.5 (and Opus 4.7 or later), and the Fable models run with it on every paid plan, including Pro; the 1M window needs no configuration and no usage credits, although Fable usage itself can bill to credits on some plans. Only Opus 4.6 and Sonnet 4.6 need their `[1m]` variant, with plan-dependent usage credits. It is not available on the free tier, and the 1M window carries no price premium for tokens beyond 200K.
