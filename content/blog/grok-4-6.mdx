...
**Grok 4.6 is xAI's frontier AI model, released on August 12, 2026, and built for long-running agents, coding, and knowledge work.** It is a version of [Grok](https://geotoolbox.ai/blog/what-is-grok), the [large language model](https://geotoolbox.ai/blog/how-does-chatgpt-work) assistant made by xAI, the company Elon Musk founded in 2023 and which now operates as SpaceXAI after the 2026 SpaceX merger. If you have been tracking the version numbers, 4.6 lands about a month after [Grok 4.5](https://geotoolbox.ai/blog/grok-4-5) and held the flagship slot for six weeks until **Grok 4.7** superseded it on September 21, 2026, at the same price.
...
<tr><td><strong>Availability</strong></td><td>xAI API, Grok Build (default), Cursor (all plans), Microsoft Office add-ins, OpenRouter/Vercel/Cloudflare gateways; also grok.com and the iOS/Android apps on the $30/mo SuperGrok plan</td></tr>
...
Grok 4.6 launched as a developer product and, as of this writing (September 2026), has since moved onto consumer surfaces too. Per xAI's model card, it is available through the xAI API (as `grok-4.6`), as the default model in Grok Build, in Cursor on every plan tier, and, notably, as the default model in the Microsoft Word, PowerPoint, and Excel add-ins, a direct move onto Copilot's turf. It is also routable through OpenRouter, Vercel, Cloudflare, and other gateways.

On the consumer side, [xAI's pricing page](https://x.ai/pricing) lists "Grok 4.6 model" as a feature of the $30-a-month SuperGrok plan, on grok.com and the iOS/Android apps, so if you are asking whether Grok 4.6 is the model answering you there today, the answer is yes. Whether the version embedded directly inside the X app has caught up is less clearly documented, so do not assume X-native access matches the standalone app. For the full SuperGrok tier lineup, including the $100-a-month SuperGrok Plus tier, see our [Grok pricing guide](https://geotoolbox.ai/blog/grok-pricing).
...
Two more factors belong in the decision. On governance, xAI's model card documents autonomous behavior in detail but includes little formal autonomy-risk evaluation, which is worth weighing for a regulated or enterprise deployment; and the wider picture matters too, since X and the earlier xAI organization face open UK and EU investigations (Ofcom, the ICO, and a formal EU DSA probe) that concern the platform's history rather than any finding against the Grok 4.6 API. On timing, xAI shipped **Grok 4.7** on September 21, 2026, at the same $2/$6 price and speed, through the xAI API, Cursor, Grok Build, and third-party harnesses, routers, and clouds; if you are deciding today, that is the newer option to evaluate first. For what is known about the next jumps, see [what we know about Grok 5](https://geotoolbox.ai/blog/grok-5).

## Frequently Asked Questions

### Is Grok 4.6 free?

No, but it is not developer-only anymore either. It launched on developer surfaces (the API, Cursor, Grok Build, and the Office add-ins) and has since reached consumer chat: xAI's pricing page lists Grok 4.6 as part of the $30-a-month SuperGrok plan on grok.com and the iOS/Android apps. xAI's launch page also advertises a free Grok Build trial, though coverage puts Grok Build inside the same SuperGrok plan, so check x.ai/build before assuming free access. API use is paid.
...
Grok 4.7 already arrived: xAI released it on September 21, 2026, at the same $2/$6 price and speed as 4.6. Grok 5 is a separate, larger model still in training; no primary source gives it a release date, so treat any specific timeline for it as speculation.
