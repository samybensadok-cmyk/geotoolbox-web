...
Two older details still confuse people. DeepSeek's earlier off-peak discount, the V3-era window that knocked 50 to 75% off during quiet hours, no longer appears on the current pricing, which is flat, with no time-of-day discount to schedule around. And the peak-hour surcharge that circulated after V4's launch is not on the pricing page either: as of August 2026 the page shows flat all-day rates, with one forward warning that DeepSeek plans to "raise the overall pricing" for the API "in the near future, with a significant increase expected," the specifics "subject to official notice." That is a rare upward signal, so the move that never fails applies double here: check the date on any DeepSeek price before you trust it.
