...
ChatGPT pulls Reddit constantly and almost never credits it. In the same analysis, [67.8% of all non-cited URLs came from Reddit](https://ahrefs.com/blog/why-chatgpt-cites-pages/), even though Reddit's own citation rate was just 1.93%. Independent network-traffic analysis backs the same pattern. When Suganthan Mohanadasan [read ChatGPT's own network traffic](https://suganthan.com/blog/how-chatgpt-picks-sources/) across a few dozen searches on one logged-in Pro account in June 2026, Reddit was fetched 278 times and cited just 11, while YouTube was fetched 201 times and cited not once.

His read on YouTube's zero is mechanical: a fetched YouTube page hands back metadata, not the video transcript, so there is little for a citation to bind to; a fetched Reddit thread, by contrast, arrives as full readable text the model can quote. The model mines those Reddit threads for context, opinions, and phrasing, then writes the answer and cites a cleaner-looking source instead.

This is the gap most people miss: retrieval is not citation. Suganthan's traffic analysis splits it into three states worth naming. **Fetched** means ChatGPT's retrieval tool pulled your page into the model's working context. **Cited** means a numbered footnote pins your page to one specific claim, the link you can see. **Mentioned** means your brand name turns up in the answer while a different page gets the citation. That is narrower than a plain brand mention, where you can be both named and cited. The states come apart constantly: a page can be fetched and never cited, so it can shape the wording of an answer while never surfacing in the Sources panel. That hidden layer is exactly what the panel undercounts.
...
- Suganthan Mohanadasan - How ChatGPT Actually Picks Sources (network-traffic analysis, June 2026) - `suganthan.com/blog/how-chatgpt-picks-sources`
