...
ChatGPT est-il donc une IA agentique ? En soi, non. Le modèle de base est une IA générative. Activez ChatGPT Work, le mode agentique d'OpenAI (lancé en juillet 2026, en train de remplacer progressivement l'ancien « mode agent » depuis le mois suivant), et connectez-le à des outils, et ce même modèle se met à se comporter de façon agentique. L'étiquette dépend de ce que le système sait faire, pas de son nom de marque. Les mécanismes sous-jacents sont détaillés plus bas dans cet article.
...
<figure className="not-prose my-8">
  <BlogImage src="/blog/ia-agentique/boucle-agent-ia.png" alt="La boucle en quatre étapes d'un agent IA : percevoir, raisonner, agir, apprendre." w="1040" h="354" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
...
2. **Raisonner et planifier.** Un grand modèle de langage (LLM), le moteur qui [prédit le token suivant](https://geotoolbox.ai/fr/blog/comment-fonctionne-chatgpt), interprète ces informations, déduit ce qu'exige l'objectif et décide de l'étape suivante.
...
<figure className="not-prose my-8">
  <BlogImage src="/blog/ia-agentique/fiabilite-calcul.png" alt="Graphique montrant comment une précision de 95 %, 90 % et 85 % par étape se traduit par environ 60 %, 35 % et 20 % de réussite sur une tâche d'agent en 10 étapes." w="1040" h="350" sizes="(min-width: 768px) 768px, 100vw" className="w-full h-auto rounded-2xl border border-[var(--surface-warm-border)]" />
  <figcaption className="mt-3 text-center text-sm text-gray-500">La longueur de la barre est la probabilité que les 10 étapes s'enchaînent sans un seul échec.</figcaption>
</figure>
