...
- Il tourne sur les modèles Claude (Opus 5, Sonnet 5) ; vous changez de modèle avec la commande `/model`, et la même boucle agentique s'exécute en dessous.
...
Concrètement : vous ouvrez un terminal dans un projet, vous tapez `claude`, et vous décrivez ce que vous voulez en une phrase. L'outil explore lui-même le code plutôt que d'attendre que vous y colliez les fichiers concernés. Pour une définition de la catégorie plus large, notre guide sur l'[agent IA](https://geotoolbox.ai/blog/agentic-ai) présente le principe général ; Claude Code en est l'une des implémentations les plus littérales.
...
Le coût, c'est que tout ce qu'il lit occupe le même budget. Vos instructions, les fichiers ouverts, la sortie de chaque commande et l'intégralité de la conversation partagent une seule [fenêtre de contexte](https://geotoolbox.ai/blog/how-does-ai-search-work), et elle se remplit plus vite que la plupart des gens ne l'imaginent. Nous avons détaillé ce mécanisme dans notre guide sur la [fenêtre de contexte de Claude Code](https://geotoolbox.ai/blog/claude-code-context-window), notamment comment lire la jauge avant que la qualité de la sortie ne commence à se dégrader.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>Mode rapide</strong></td><td>10 $ / 50 $ par million de tokens (entrée / sortie)</td><td>Préversion de recherche, forfaits à la consommation</td></tr>
  </tbody>
</table>
...
<Callout type="note">
À noter pour la France : les AI Overviews et le Mode IA (AI Mode) de Google **sont déployés en France depuis le 22 juillet 2026**, une fois réglée la question des droits voisins. Le test ci-dessus porte sur les résultats américains ; le même risque de citation obsolète s'applique désormais au marché français, et il joue aussi via les assistants que les développeurs français utilisent au quotidien (ChatGPT, Perplexity, Mistral).
</Callout>
...
- Présentation de Claude Code (Anthropic, documentation Claude Code) - `code.claude.com/docs/en/overview`
- Configurer Claude Code (Anthropic, documentation Claude Code) - `code.claude.com/docs/en/setup`
- Fonctionnement de Claude Code (Anthropic, documentation Claude Code) - `code.claude.com/docs/en/how-claude-code-works`
- Gérer efficacement les coûts (Anthropic, documentation Claude Code) - `code.claude.com/docs/en/costs`
- Utilisation des données (Anthropic, documentation Claude Code) - `code.claude.com/docs/en/data-usage`
- Bonnes pratiques sur les limites d'usage (Anthropic, Assistance Claude) - `support.claude.com/en/articles/9797557-usage-limit-best-practices`
- Page produit Claude Code (Anthropic) - `claude.com/product/claude-code`
- anthropics/claude-code (Anthropic, GitHub) - `github.com/anthropics/claude-code`
- Fiche du plugin Claude Code [Beta] (Anthropic, via JetBrains Marketplace) - `plugins.jetbrains.com/plugin/27310-claude-code-beta-`
- L'outillage IA en 2026 (Gergely Orosz et Elin Nilsson, The Pragmatic Engineer, mars 2026) - `newsletter.pragmaticengineer.com/p/ai-tooling-2026`
- Méga-fil sur les limites d'usage de Claude (r/ClaudeAI) - `reddit.com/r/ClaudeAI/comments/1s7fcjf/claude_usage_limits_discussion_megathread_ongoing`
- Claude Code envoie 33 000 tokens avant de lire le prompt ; OpenCode en envoie 7 000 (Hacker News, juillet 2026) - `news.ycombinator.com/item?id=48883275`
