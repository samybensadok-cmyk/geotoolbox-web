...
Le nombre qui décide de tout, c'est 1,56 To, la taille du [dépôt Kimi K3](https://huggingface.co/moonshotai/Kimi-K3) sur Hugging Face, réparti sur 96 fragments de poids. Un chiffre plus petit, autour de 594 Go, circule dans une partie des articles publiés au lancement. Ce n'est pas le modèle complet, c'est la taille de la quantification GGUF 1-bit d'Unsloth. L'arithmétique sur les poids complets écarte tout chiffre plus petit : 594 Go impliqueraient environ 1 200 milliards de paramètres en 4 bits, or K3 en compte 2 800 milliards, donc la route des poids officiels exige toujours de budgéter 1,56 To.
...
Aucune configuration domestique ne convient en pleine précision. Même un M3 Ultra de 512 Go reste loin des 1,56 To dont les poids officiels ont besoin, et une RTX 4090 (24 Go de VRAM) n'en couvre qu'environ un soixante-cinquième. La quantification GGUF 1-bit de 594 Go d'Unsloth (environ 79 % de précision conservée) est la seule version qui tient dans une seule machine de 512 Go avec de la marge pour le cache KV, via un déchargement disque ou RAM, mais c'est un modèle fortement dégradé, pas celui que ce guide teste. Le plancher réel en pleine précision est un nœud de datacenter, 8× B300 (2,3 To) ou 16× B200. Un cluster de quatre à cinq Mac Studio de 512 Go peut tenir le modèle en pleine précision, mais le lien Thunderbolt entre eux est une centaine de fois plus lent que la mémoire interne de chaque machine, donc l'interconnexion fixe la vitesse. Personne n'a publié d'exécution de K3 sur un cluster de Mac ; l'analogie la plus proche, un montage de génération précédente testé sur d'autres grands modèles, tournait autour de 15 tokens par seconde.

### Ollama et llama.cpp gèrent-ils Kimi K3 ?

llama.cpp, oui ; Ollama, toujours pas en local. llama.cpp en version mainline a obtenu la prise en charge de K3 via une [fusion le 15 août 2026](https://github.com/ggml-org/llama.cpp/pull/26185), le lendemain de la première publication de ce guide, donc les dépôts GGUF n'exigent plus de compilation depuis une branche annexe. Une suite le 8 septembre 2026 ([PR #28466](https://github.com/ggml-org/llama.cpp/pull/28466)) a activé le rollback d'état récurrent de K3 pendant le décodage spéculatif, une fonctionnalité de performance de service que vLLM et SGLang avaient déjà dès leur lancement au jour zéro. Les quantifications GGUF dynamiques d'Unsloth vont d'une version 1-bit de 594 Go (environ 79 % de précision conservée) jusqu'à une version Q8 sans perte de 1,56 To. L'étiquette « cloud » d'Ollama route toujours vers les serveurs d'Ollama plutôt que de tourner en local, et il n'existe pas encore de build Ollama dédié, donc llama.cpp (ou une interface construite dessus, comme LM Studio) reste la seule voie locale de type Ollama aujourd'hui.

### Quelle taille fait le téléchargement de Kimi K3 ?

Le dépôt officiel pèse 1,56 To sur 96 fragments de poids, en précision native MXFP4 4 bits. Le chiffre de 594 Go souvent répété n'est pas un chiffre à part ou erroné : c'est la taille de la quantification GGUF dynamique 1-bit d'Unsloth (UD-IQ1_S), qui conserve environ 79 % de précision selon le benchmark d'Unsloth. Comme K3 a été entraîné avec quantification intégrée en 4 bits, il n'y a pas de division gratuite par deux à espérer du passage habituel de 16 à 4 bits ; la version 2-bit d'Unsloth (UD-Q2_K_XL) pèse encore environ 861 Go, et sa version Q8 sans perte atteint les 1,56 To complets. Les quantifications communautaires sous 4 bits sacrifient de la qualité pour réduire la taille plutôt que de compresser un original gonflé.
...
- Quantifications GGUF et benchmarks de Kimi K3 - Documentation Unsloth - `unsloth.ai/docs/models/kimi-k3`
- model: add Kimi-K3 text model (PR #26185, fusionnée le 15 août 2026) - llama.cpp / GitHub - `github.com/ggml-org/llama.cpp/pull/26185`
- model: support Kimi-K3 recurrent-state rollback (PR #28466, fusionnée le 8 septembre 2026) - llama.cpp / GitHub - `github.com/ggml-org/llama.cpp/pull/28466`
