---
title: "Claude Opus 5 : prix en euros, benchmarks et ce qui change vraiment"
description: "Le vrai prix de Claude Opus 5 en euros, les cinq niveaux d'effort qui peuvent doubler la facture, et ce qui casse quand vous migrez depuis Opus 4.8."
date: "2026-07-25"
updated: "2026-09-25"
...
**Mise à jour du 22 septembre 2026 :** [Claude Opus 5.5](https://www.anthropic.com/news/claude-opus-5-5) a remplacé Opus 5 comme modèle Opus actuel d'Anthropic, à 4 $ en entrée et 20 $ en sortie par million de tokens, sous les 5 $ et 25 $ d'Opus 5. Opus 5 reste disponible comme modèle précédent, au même tarif qu'au lancement. Tout ce qui suit décrit Opus 5 tel qu'il est sorti et s'applique à ce modèle précisément ; voir notre [guide des prix Claude](https://geotoolbox.ai/fr/blog/prix-claude) pour la gamme actuelle.
...
Il s'agit d'un [modèle de raisonnement](https://geotoolbox.ai/blog/how-ai-models-think) ; au lancement, c'était le modèle par défaut sur Claude Max et le modèle le plus puissant disponible sur Claude Pro. Opus 5.5 a depuis repris les deux rôles, et depuis Claude Code 2.1.280 le modèle par défaut est Opus 5.5 sur Pro, Max, Team et Enterprise (Pro et Team Standard utilisaient auparavant Sonnet 5 par défaut). Il n'est pas inclus dans la formule gratuite.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>Date de coupure des connaissances</strong></td><td>Mai 2026</td></tr>
...
Ensuite, l'effort n'est pas une nouveauté d'Opus 5. Le même paramètre fonctionne sur Opus 4.5 et les versions suivantes, ainsi que sur Sonnet 5 et Fable 5. Ce qui a changé, c'est le coût de ce choix, l'objet de la section suivante. Si vous avez repris des réglages d'un modèle antérieur, la documentation est claire : refaites une analyse par paliers sur vos évaluations au lieu de réutiliser ces réglages, et l'effort ne raccourcit pas de façon fiable les réponses visibles sur Opus 5. Si vous voulez des réponses plus courtes, demandez des réponses plus courtes.
...
La colonne Sonnet 5 (2 $ / 10 $) est le tarif permanent : Anthropic l'avait d'abord annoncé comme tarif de lancement jusqu'au 31 août 2026, puis a annulé la hausse prévue au 1er septembre à 3 $ et 15 $, de sorte que l'écart avec Opus 5 se maintient.

Trois détails de la colonne Opus 5 méritent qu'on s'y arrête.
...
**Le mode rapide est un choix produit distinct.** Il tourne environ 2,5 fois plus vite pour le double du prix, sous forme de préversion, limitée à l'API Claude native. L'API Batch et les clouds partenaires ne le proposent pas.

**La taille minimale d'un prompt pouvant être mis en cache est descendue à 512 tokens**, contre 1 024 sur Opus 4.8. Si vous mettez en cache de courts prompts système qui passaient sous le seuil, ils deviennent éligibles.
...
## Le prix du token est stable. Le coût par tâche, pas forcément.

« Même prix que l'Opus 4.8 » : c'est le titre que tout le monde a repris. C'est vrai à l'échelle du token, et ce n'est pas toute la facture.
...
[Artificial Analysis](https://artificialanalysis.ai/articles/opus-5), qui fait tourner sa propre suite d'évaluation indépendante, l'a mesuré. Son constat : à effort max, Opus 5 « coûte en moyenne 2,03 $ par tâche de l'Intelligence Index, en dessous de Claude Fable 5 (avec repli) à 2,75 $, mais toujours au-dessus de Claude Opus 4.8 (max) à 1,80 $ et de Claude Sonnet 5 (max) à 1,53 $ ».
...
**Anthropic signale ses propres deuxièmes places. L'annonce, non.** La System Card est franche là où le communiqué ne l'est pas. Fable 5 mène sur SWE-bench Pro (80 contre 79,2), GPT-5.6 Sol devance Opus 5 sur DeepSWE v1.1 (72,7 contre 68,8) et HealthBench Professional (60,5 contre 59,8), et FrontierCode est de fait à égalité, 53,5 contre 53,4 en faveur de Fable. Cela fait au moins quatre lignes où Opus 5 n'est pas premier, dans un lancement centré sur le codage agentique, et vous ne les trouvez qu'en ouvrant le PDF. Traitez toutefois ces quasi-égalités comme des égalités : la System Card configure l'effort par benchmark, donc un dixième de point n'est pas un classement.

## Ce qui s'améliore vraiment par rapport à Opus 4.8

Une fois le bruit des benchmarks écarté, cinq changements reviennent régulièrement, autant dans les documents du fournisseur que dans les retours de praticiens.
...
**Fraîcheur des connaissances.** La date de coupure de mai 2026 est plus récente de quatre mois que celle de janvier 2026, valable pour Opus 4.8 comme pour Fable 5. Pour un modèle qui répond à des questions sur son propre écosystème, cet écart pèse plus qu'il n'y paraît.
...
**Sortie visuelle.** Anthropic a mis en avant une simulation en soufflerie et une illustration interactive de cellule entièrement produites par le modèle, et des clients qui construisent sur ce terrain ont rapporté les meilleurs travaux d'animation et de 3D jamais vus de la part d'un modèle Opus.
...
Une amélioration est allée dans l'autre sens, et c'est le chiffre le moins confortable du lancement. La System Card d'Anthropic indique que le modèle « produit un peu plus d'affirmations factuelles fausses qu'Opus 4.8, tout en étant globalement plus précis ». Artificial Analysis a mesuré le même arbitrage : sur son benchmark AA-Omniscience, Opus 5 gagne 7 points de précision factuelle sur Opus 4.8, mais « répond plus souvent en cas d'incertitude », son taux d'hallucination grimpant de 14 points, à 50 %. Le surcroît de connaissances et l'aplomb accru des réponses fausses vont de pair. Si vous destinez ce modèle à de la recherche ou à du travail de connaissance, c'est la réserve qui compte le plus, et celle que les tableaux de lancement n'affichent pas.

## Les garde-fous se sont desserrés, et c'est là le vrai enjeu pour l'entreprise
...
**Les refus ne sont pas des erreurs.** Quand un classificateur de sécurité décline une requête, la documentation est explicite : « les classificateurs de sécurité renvoient ce motif d'arrêt sous forme de réponse HTTP 200 normale, pas d'une erreur », avec `stop_reason` réglé sur `refusal` et un objet `stop_details` nommant la catégorie de politique. Une gestion d'erreurs qui ne guette que les codes d'état différents de 200 passera complètement à côté. Anthropic note qu'une requête refusée sur Opus 5 ou Fable 5 peut en général être servie en réessayant sur un autre modèle Claude.
...
**Vous n'aurez peut-être pas le contexte d'un million de tokens dont vous avez entendu parler.** Les utilisateurs du premier jour rapportaient voir environ 200 000 tokens via Claude Code, et au lancement l'explication se trouvait dans la documentation de Claude Code plutôt que dans l'article de lancement : sur les formules Max, Team et Enterprise, Opus était « automatiquement mis à niveau vers un contexte d'un million de tokens, sans configuration supplémentaire », tandis que sur Pro le contexte étendu « nécessite des crédits d'usage ». Ce partage par formule ne s'applique plus. La [documentation de configuration des modèles](https://code.claude.com/docs/en/model-config) d'Anthropic (consultée le 26 septembre 2026) indique qu'Opus 4.7 et les versions suivantes, Opus 5 compris, tournent avec la fenêtre d'un million sur toutes les formules, Pro inclus, sans variante `[1m]` ni crédits d'usage. Pour utiliser Opus 5 lui-même plutôt que l'alias `opus`, qui pointe désormais vers Opus 5.5, tapez `/model claude-opus-5`. Un administrateur peut toujours désactiver le million entièrement avec `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`, qui ramène les modèles nativement à un million à 200 000 tokens. La fenêtre de contexte elle-même n'entraîne aucun surcoût au-delà de 200 000 tokens.

Le million de tokens est donc bien réel, et sur les versions actuelles de Claude Code il ne dépend plus de votre formule. Si le comportement en contexte long compte pour vous, vérifiez sur quel modèle vous êtes réellement avant de vous engager, et voyez notre guide sur les [limites de la fenêtre de contexte](https://geotoolbox.ai/blog/claude-code-context-window) pour comprendre pourquoi la [fenêtre de contexte](https://geotoolbox.ai/blog/how-does-ai-search-work) annoncée et celle réellement utilisable divergent si souvent.

Changer la valeur d'effort entre deux requêtes invalide la mise en cache des prompts, car l'effort façonne le prompt rendu. Choisissez un niveau au début d'une session mise en cache et gardez-le.
...
<table>
  ...
  <tbody>
    <tr><td><strong>Prix (entrée / sortie)</strong></td><td>5 $ / 25 $</td><td>10 $ / 50 $</td><td>2 $ / 10 $ (tarif permanent)</td></tr>
    ...
    <tr><td><strong>Date de coupure des connaissances</strong></td><td>Mai 2026</td><td>Janvier 2026</td><td>Janvier 2026</td></tr>
...
Alors, Opus 5 est-il meilleur que Fable 5 ? Pour la plupart des tâches, oui, pour un prix par token divisé par deux, et il devance Fable 5 sur la plupart des lignes du propre tableau d'Anthropic. Mais la System Card s'ouvre sur l'affirmation inverse, noir sur blanc : « Claude Opus 5 n'est pas globalement plus performant que notre modèle à accès général le plus performant, Claude Fable 5. » Fable reste en tête sur SWE-bench Pro, et la documentation continue de lui confier les charges à plus haute capacité. Anthropic ne concilie pas les deux à votre place.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>Lecture de la personnalité</strong></td><td>Timide et trop porté à s'excuser ; dans sa vidéo d'accompagnement : « Je déteste Opus 5. C'est quand même le meilleur modèle. »</td><td>Insistant, tranché, enclin à la contradiction</td></tr>
...
<table>
  ...
  <tbody>
    <tr><td>GPT-5.6 Terra (OpenAI API)</td><td>Activée</td><td>Correct</td></tr>
    ...
    <tr><td>GPT-5.6 Terra (OpenAI API)</td><td><strong>Désactivée</strong></td><td>Échec, n'a rien pu vérifier</td></tr>
...
La récupération n'est pas une garantie pour autant, et notre propre panel a fourni le contre-exemple. L'un de ces cinq a chiffré la fenêtre de contexte à 200 000 tokens, deux fois, dans un tableau en gras, en citant Reuters, The Verge et Axios pour un chiffre qu'aucun ne portait. Les quatre autres ont dit un million. La récupération a réglé le problème de fraîcheur. Elle n'a pas réglé celui de l'aplomb.
...
Le même jour, `claude-opus-5` était encore absent du catalogue de modèles LLM de DataForSEO, tout à fait le genre de latence d'infrastructure qui décide si un outil parvient ne serait-ce qu'à voir un nouveau modèle.

Rien de tout cela n'est un reproche aux moteurs. C'est le mécanisme à l'œuvre derrière chaque réponse d'IA sur tout sujet récent : la réponse est un **résultat de récupération, pas un souvenir**. Mêmes poids, même prompt, même session. La seule variable était de savoir si le modèle pouvait aller chercher l'information, et cette variable a produit l'écart entre une réponse correcte et une réponse [hallucinée](https://geotoolbox.ai/blog/ai-hallucinations) avec assurance.
...
### Claude Opus 5 est-il disponible dans la formule gratuite ?

Non. Claude Opus 5 exige une formule payante. Au lancement, c'était le modèle par défaut sur Claude Max et le modèle le plus performant disponible sur Claude Pro ; Opus 5.5 occupe désormais les deux positions, et depuis Claude Code 2.1.280 c'est le modèle par défaut sur Pro, Max, Team et Enterprise. Opus 5 reste accessible sur Team, Enterprise, Claude Code et l'API comme modèle précédent. Les utilisateurs de la formule gratuite ont Claude Sonnet 5 à la place.
...
Oui, et `high` y est aussi le défaut. Sur l'API Anthropic, Opus 5 tourne avec la fenêtre d'un million sur toutes les formules payantes, Pro inclus, sans variante `[1m]` ni crédits d'usage (au lancement, Pro exigeait des crédits d'usage). Sélectionnez-le avec `/model claude-opus-5`, puisque l'alias `opus` pointe désormais vers Opus 5.5 ; la fenêtre n'entraîne aucun surcoût au-delà de 200 000 tokens.
...
Opus 5 n'est pas vendu séparément. Il était inclus dans Claude Pro à 20 $ par mois (environ 21 € TTC en France), où il était le modèle le plus performant disponible, et dans Claude Max à 100 $ ou 200 $ par mois (environ 105 € ou 210 € TTC), où il était le modèle par défaut ; Opus 5.5 est depuis devenu le modèle par défaut sur Pro, Max, Team et Enterprise. Les formules Team et Enterprise incluent Opus. Payer au token via l'API coûte, pour Opus 5 précisément, 5 $ par million en entrée et 25 $ par million en sortie, sur une facture entièrement distincte de l'abonnement.

### Vaut-il le coup de passer d'Opus 4.8 à Claude Opus 5 ?

Le prix par token ne change pas, mais le gain de capacité n'est pas automatiquement gratuit : la réflexion est désormais activée par défaut et les réponses sont plus longues, donc une même tâche peut consommer plus de tokens. Testez votre propre charge de travail à `medium` et `high` avant de supposer que la facture reste stable, et prévoyez du temps pour les ruptures de compatibilité autour de `thinking` et `max_tokens`.
...
- Claude Opus 5.5 - Anthropic, 22 septembre 2026 - `anthropic.com/news/claude-opus-5-5`
- Journal des modifications de Claude Code (2.1.280) - Anthropic - `code.claude.com/docs/en/changelog`
- Configuration des modèles (documentation Claude Code) - Anthropic, consultée le 26 septembre 2026 - `code.claude.com/docs/en/model-config`
