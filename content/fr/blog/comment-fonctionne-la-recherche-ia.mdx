...
4. **Génération avec citation.** Le modèle prend les sources survivantes, remplit sa fenêtre de contexte avec leur texte et produit une réponse ancrée dans ce texte. Les liens affichés sous la réponse sont des citations visibles, sans offrir une traçabilité parfaite de la provenance. Ce schéma d'ensemble a un nom : la **génération augmentée de récupération** (RAG). Il vaut la peine de comprendre [ce qu'est le RAG et comment devenir la page qu'il récupère](https://geotoolbox.ai/blog/what-is-rag).
