...
- Sa faille de fond est l'injection indirecte de prompt : six équipes de sécurité (Brave, LayerX, Trail of Bits, Zenity, Guardio, Hacktron) ont démontré des attaques réelles, plusieurs avec exfiltration de données. Ne connectez pas de comptes sensibles à l'agent.
...
### Perplexity Comet est-il sûr ?
Pour de la lecture et du résumé à faible enjeu, le risque est modéré. Pour les tâches agentiques, soyez prudent. Des chercheurs de Brave, LayerX, Trail of Bits, Zenity, Guardio et Hacktron ont tous démontré des attaques par injection de prompt ou au niveau des extensions, où des instructions cachées dans une page détournent l'assistant. Évitez de lancer l'agent dans des sessions avec des données bancaires, médicales ou professionnelles tant que le problème n'est pas mieux maîtrisé.
