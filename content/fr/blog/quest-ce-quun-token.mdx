...
L'arithmétique déraille pour une raison voisine. Les nombres longs sont coupés en tokens à des endroits arbitraires, si bien que « 1234567 » peut se scinder en morceaux qui ne s'alignent pas pour un calcul chiffre par chiffre. C'est en partie pour cela que des modèles ont hésité sur des questions comme « 9,11 est-il plus grand que 9,9 ? ». Et le calcul effectué par token est de quantité fixe, sans rapport avec la suite d'étapes, aussi nombreuses qu'il le faut, qu'on enchaînerait pour poser les retenues d'une longue addition.

Les modèles récents s'en sortent mieux, mais il faut dire pourquoi avec soin. La parade habituelle consiste à faire épeler le mot, ou à espacer les lettres, ce qui simplifie la tâche au niveau des caractères. C'est un contournement, pas un correctif au niveau du tokeniseur. Fin 2025, [des tests prenaient encore GPT-5.2 en flagrant délit de mauvais comptage](https://dataconomy.com/2025/12/15/gpt-5-2-still-counts-two-rs-in-strawberry/) sur certaines variantes (les modèles de premier plan ont depuis évolué, mais le problème est structurel, pas passager). C'est la même famille de failles qui produit d'autres [hallucinations de l'IA](https://geotoolbox.ai/fr/blog/hallucination-ia) : le modèle affirme avec aplomb une chose que son architecture ne lui a pas permis de vérifier.
...
Mi-2026, [plus d'une douzaine de modèles de pointe proposent des fenêtres d'un million de tokens ou plus](https://artificialanalysis.ai/models), à l'échelle mondiale, et le plus grand modèle à poids ouverts en annonce 10 millions. Une mise en garde que le marketing oublie : le contexte réellement exploitable est plus petit que le chiffre affiché, donc une fenêtre d'un million de tokens ne garantit pas un million de tokens d'attention fiable.
