...
Vous verrez des fournisseurs comparer les modèles sur le nombre de dimensions. Plus de dimensions ne veut pas dire, à lui seul, une meilleure représentation du contenu, et de toute façon vous ne choisissez pas ce qu'utilisent les moteurs en production. Rangez ce chiffre dans « comment ça marche », pas dans « ce qu'il faut optimiser ».
</Callout>
...
Une exception documentée à noter : [Google indique](https://developers.google.com/search/docs/fundamentals/ai-optimization-guide) que le grounding des AI Overviews remonte les candidats via ses systèmes de classement Search principaux (le Mode IA y ajoutant le query fan-out), donc les embeddings travaillent à l'intérieur du classement classique plutôt que comme un index vectoriel autonome. ChatGPT Search et Perplexity semblent plus proches du pipeline ci-dessus, d'après le comportement observé ; les rouages internes de la recherche web en direct ne sont pas entièrement publiés.
...
**Être récupéré n'est pas être cité.** La similarité d'embedding vous fait entrer dans la liste restreinte à l'étape 2. Elle ne décide pas qui est crédité à l'étape 4. Un passage peut être récupéré et jamais cité, doublé au dernier moment par un concurrent plus clair ou faisant davantage autorité. Les embeddings décident qui est dans la pièce, pas qui est cité.
...
Il n'y a pas non plus de cible stable à viser. Les moteurs changent de modèle d'embedding, et un changement veut dire ré-encoder le contenu que le nouveau modèle explore. Un contenu réglé sur les particularités d'un modèle n'hérite de rien quand le modèle change.
...
La récupération vous fait entrer dans la pièce. La dernière étape, quel passage est nommé comme source, dépend de couches de classement sur lesquelles les moteurs publient encore moins.
...
Vous en savez maintenant plus sur les mécanismes de récupération que ce que suppose une bonne part de ce qui se vend comme « optimisation IA ».
